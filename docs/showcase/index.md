...
进入主终端时，AK 标识从倾斜状态归正、信号线展开，背景由虚化转为清晰，人物、博士档案和左右菜单依次出现，约 2.4 秒完成。展开「区块入场 · 独立重播」可以单独播放 10 个区块，等级、资源和理智数值随对应区块增长。[入场与数字效果组件](/components/ak-fx#区块入场与数字增长)可在其他页面复用。点击终端左上方「重播入场」可再次查看，原有景深与菜单状态保留。竖屏与系统「减少动态效果」模式直接展示静态界面。

动效节奏参考 [2086丷《有空再做了【明日方舟】》](https://www.bilibili.com/video/BV1Qj411x745/)，以 CSS 和 Web Animations API 重新实现标识、线条与分层入场，使用本项目的 AK 标识和已有素材。

### 景深动画播放

[打开景深动画播放器](#depth-playground)，可播放或暂停 8 秒分层动画，拖动进度逐段检查，或切换循环播放。动画期间由时间轴控制景深；点击「返回指针模式」恢复指针交互和原始人物缩放。关闭景深或开启系统减少动态效果偏好，会停止播放并恢复静态状态。
